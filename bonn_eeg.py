...
def load_bonn_data(csv_path='bonn_eeg_preprocessing/bonn_eeg_data.csv'):
    data = pd.read_csv(csv_path)
    label_map = {'Z':0,'O':1,'N':2,'F':3,'S':4}
    y = data['label'].map(label_map).values
    X = data.drop(columns=['label'])
    X_train, X_test, y_train, y_test = train_test_split(X.values, y, test_size=TEST_SIZE, random_state=42, stratify=y)
...
def run_patx():
    ...
    model = get_model(TASK_TYPE, 'BONN', n_classes)
    optimizer = PatternOptimizer(
        ...
        dataset='BONN', multiple_series=len(TIME_SERIES_IDENTIFIERS) > 0,
    ...
    t1 = time.time()
    optimizer.save_parameters_to_json('BONN')
...
def run_tsfresh():
    ...
    m = get_model(TASK_TYPE, 'BONN', n_classes)
...
print("Running BONN EEG baselines: PATX, TSFRESH, CNN")
results = []
...
df.to_csv('results/bonn_eeg.csv', index=False)