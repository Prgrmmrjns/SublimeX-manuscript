...
B--spline templates across multiple signal transformations (e.g.\ raw, cumulative
sum, DCT, FFT power, wavelet), patX learns compact, highly predictive patterns
that serve as
...
We evaluate patX on seven diverse biomedical tasks including glucose forecasting, 
arrhythmia classification and gene expression prediction.
...
of signal transformations and automatically selects the most informative
representations before pattern discovery.
...
We evaluate patX across seven biomedical tasks spanning arrhythmia
classification on MIT-BIH \cite{MoodyMark1992,Goldberger2000}, emotion
classification from EEG (Emotions), gene expression prediction from histone
modification profiles (REMC) \cite{Roadmap2015}, ARDS prediction from MIMIC-IV
clinical time series \cite{Johnson2023MIMICIV}, glucose forecasting (AZT1D),
human activity recognition from IMU sensors (PAMAP2) \cite{Reiss2012PAMAP2}, and
voice pathology detection (SVD).
We compare against established automated feature baselines (tsfresh, catch22)
and a CNN baseline under matched splits and metrics.

Our contributions are:
(i) a novel freely available open-source framework for pattern discovery in (biomedical) signals that is easy 
to implement, individualize and extended upon a users specific needs,
(ii) an evaluation across heterogeneous tasks illustrating that a small number
of patterns can achieve competitive performance, and
(iii) a general-purpose framework that learns interpretable spline patterns as
explicit features for biomedical signals opening up new opportunities for knowledge discovery in (biomedical) datasets.
...
Compact, human-readable patterns; localization of best match; adapts via
transform selection &
...
library and automatic transform selection, allowing patterns to emerge in the
domain most predictive for a given biomedical task.
...
a three-stage pipeline: (1) signal transformation, (2) transform selection, and
(3) iterative pattern discovery.
...
The default transformation library includes time-domain (raw, delta-to-first
sample, percent change, cumulative sum, difference), frequency-domain (DCT, FFT
power), wavelet decompositions (Coiflet-1), and nonlinear transforms (log1p,
absolute value, softsign, reciprocal, sorting, pre-emphasis).
...
Trials per pattern & 1,000 \\
B-spline Degree & Quadratic (degree 2) \\
Control Points & 3 per pattern \\
Transforms Library & 13 built-in (time/frequency/wavelet/nonlinear) \\
...
The built-in transformation library includes: raw, delta0, pct\_change, cumsum,
diff, log1p, abs, softsign, sorted, DCT, reciprocal, preemph99, FFT power, and
a Coiflet-1 wavelet transform.
...
including moments, spectral features, and shape statistics, training LightGBM on
...
tsfresh + LightGBM, catch22 + LightGBM, and CNN baselines across seven
...
directly in the time domain without spectral or wavelet views.
...
nonlinear views: raw, delta0, pct\_change, cumulative sum, difference, log1p,
absolute value, softsign, sorted, DCT, reciprocal, pre-emphasis, FFT power, and
a Coiflet-1 wavelet transform. Each transform is applied per channel to produce
a parallel stack
...
channel, fits a spline pattern, and measures MSE-based dissimilarity in the
