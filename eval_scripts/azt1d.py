...
def load_all_azt1d():
    ...
    return (X_list, combined_df['target'], combined_df['subject_id'],
            {'initial_features': combined_df['CGM_current'].values.reshape(-1, 1),
...
def run_evaluation(X_list, y, subject_ids, info, results_path, patterns_path):
    ...
    init_train = initial_features[train_idx]
    init_test = initial_features[test_idx]
    init_feat = (init_train, init_test)
    ...
    for app in ['PATX', 'CNN', 'MiniRocket', 'catch22', 'tsfresh', 'rdst']:
        ...
        if app == 'PATX':
            res = run_patx(input_train, y_train.values, input_test, 
                          metric=metric, initial_features=init_feat)
            ...
            score, _, n_feat = eval_cnn(
                ...
                task, metric, n_classes, initial_train=init_train, 
                initial_test=init_test
            )
        elif app == 'MiniRocket':
            ...
            score, _, n_feat = eval_catch22(
                train_concat, test_concat, y_train.values, y_test.values,
                metric, n_classes, init_train, init_test
            )
        elif app == 'tsfresh':
            score, _, n_feat = eval_tsfresh(
                train_concat, test_concat, y_train.values, y_test.values,
                metric, n_classes=n_classes, initial_train=init_train, 
                initial_test=init_test
            )
        elif app == 'rdst':
            score, _, n_feat = eval_rdst(
                train_concat, test_concat, y_train.values, y_test.values,
                metric, n_classes, initial_train=init_train, 
                initial_test=init_test
            )
