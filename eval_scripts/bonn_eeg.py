...
def load_bonn_eeg_data():
    """Load Bonn EEG data as univariate time series."""
    data = pd.read_csv('../processed_datasets/bonn_eeg_data.csv')
...
# Load data once
input_series, y = load_bonn_eeg_data()
...
if os.path.exists('../results/bonn_eeg.csv'):
    existing = pd.read_csv('../results/bonn_eeg.csv')
...
if ('PATX', 1) not in done:
    ...
    os.makedirs('../json_files/bonn_eeg', exist_ok=True)
    with open('../json_files/bonn_eeg/pattern_parameters.json', 'w') as f:
...
bonn_results = pd.DataFrame(results)
for approach in ['PATX', 'TSFRESH', 'CATCH22', 'CNN']:
    approach_results = bonn_results[bonn_results['approach'] == approach]
...
pd.DataFrame(results).to_csv('../results/bonn_eeg.csv', index=False)