"""Transform composition and first-feature segment stability (Figure~2)."""
import json
import numpy as np
import pandas as pd
...
from plot_style import apply_manuscript_figure_style, mm_size, save_png_pdf_eps
from config import RESULTS, PARAMETERS, ELARTICLE

RESULTS_DIR = RESULTS
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
PARAMETERS_DIR = PARAMETERS
apply_manuscript_figure_style(13)
...
def generate_figure(transform_results, stability_results):
    ...
    fig, axes = plt.subplots(
        1, 2, figsize=mm_size(220, 82), layout='constrained',
    ...
    ax1.margins(x=0.08)
    ax1.set_ylabel('Proportion of Features', fontsize=13)
    ax1.set_ylim(0, 1.05)
    ax1.legend(
        loc='upper center', bbox_to_anchor=(0.5, 1.12),
        fontsize=7.5, ncol=4, handlelength=1.2, columnspacing=0.9,
    )
    ...
    for ds in datasets_panel_b:
        ...
        y_label_parts.append((label, cs))
        y_pos += 1

    y_labels = [f"{lbl} {cs}" for lbl, cs in y_label_parts]

    ax2.set_yticks(y_positions)
    ax2.set_yticklabels(y_labels, fontsize=11)
    ax2.set_xlim(0, 1)
    ax2.set_xlabel('Normalized Position', fontsize=13)
    ...
    save_png_pdf_eps(fig, ELARTICLE / 'feature_analysis', dpi=300)
    return fig
...
def print_summary(transform_results, stability_results):
    ...
    _ = (transform_results, stability_results)
...
def save_results_to_csv(transform_results,
    ...
    df.to_csv(RESULTS_DIR / 'performance_analysis.csv', index=False)
...
def main():
    datasets = ['pamap2', 'emotions', 'mitbih', 'remc', 'azt1d', 'mimic', 'svd']
